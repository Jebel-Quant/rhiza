...
def _validate_bundle_structure(
    bundle_name: str, bundle_config: dict, bundle_names: set
) -> list[str]:
    ...
    if "recommends" in bundle_config:
        if not isinstance(bundle_config["recommends"], list):
            errors.append(
                f"Bundle '{bundle_name}' 'recommends' must be a list"
            )
        else:
...
def validate_template_bundles(bundles_path: Path) -> tuple[bool, list[str]]:
    ...
    for bundle_name, bundle_config in bundles.items():
        errors.extend(
            _validate_bundle_structure(bundle_name, bundle_config, bundle_names)
        )
