...
def test_no_book_folder(git_repo):
    ...
    for target in ["book", "docs", "marimushka"]:
        result = subprocess.run([MAKE, "-n", target], cwd=git_repo, capture_output=True, text=True)  # nosec B603
...
def test_book_without_logo_file(git_repo):
    ...
    result = subprocess.run([MAKE, "-n", "book"], cwd=git_repo, capture_output=True, text=True)  # nosec B603
