...
def test_release_fails_if_local_tag_exists(git_repo):
    ...
    subprocess.run([GIT, "tag", "v0.1.0"], cwd=git_repo, check=True)  # nosec B603
...
def test_release_fails_if_remote_tag_exists(git_repo):
    ...
    subprocess.run([GIT, "tag", "v0.1.0"], cwd=git_repo, check=True)  # nosec B603
    subprocess.run([GIT, "push", "origin", "v0.1.0"], cwd=git_repo, check=True)  # nosec B603
...
def test_release_uncommitted_changes_failure(git_repo):
    ...
    result = subprocess.run([SHELL, str(script)], cwd=git_repo, capture_output=True, text=True)  # nosec B603
...
def test_release_pushes_if_ahead_of_remote(git_repo):
    ...
    tracked_file.touch()
    subprocess.run([GIT, "add", "file.txt"], cwd=git_repo, check=True)  # nosec B603
    subprocess.run([GIT, "commit", "-m", "Local commit"], cwd=git_repo, check=True)  # nosec B603
...
def test_release_fails_if_behind_remote(git_repo):
    ...
    subprocess.run([GIT, "clone", str(git_repo.parent / "remote.git"), str(other_clone)], check=True)  # nosec B603

    # Configure git user for other_clone (needed in CI)
    subprocess.run([GIT, "config", "user.email", "test@example.com"], cwd=other_clone, check=True)  # nosec B603
    subprocess.run([GIT, "config", "user.name", "Test User"], cwd=other_clone, check=True)  # nosec B603

    # Commit and push from other clone
    with open(other_clone / "other.txt", "w") as f:
        f.write("content")
    subprocess.run([GIT, "add", "other.txt"], cwd=other_clone, check=True)  # nosec B603
    subprocess.run([GIT, "commit", "-m", "Remote commit"], cwd=other_clone, check=True)  # nosec B603
    subprocess.run([GIT, "push"], cwd=other_clone, check=True)  # nosec B603
...
def test_dry_run_flag_recognized(git_repo):
    ...
    result = subprocess.run([SHELL, str(script), "--dry-run"], cwd=git_repo, capture_output=True, text=True)  # nosec B603
...
def test_dry_run_no_git_operations(git_repo):
    ...
    tags_before = subprocess.run(  # nosec B603
        [GIT, "tag", "-l"],
        cwd=git_repo,
        capture_output=True,
        text=True,
    ...
    tags_after = subprocess.run(  # nosec B603
        [GIT, "tag", "-l"],
        cwd=git_repo,
        capture_output=True,
        text=True,
    ...
    tag_check = subprocess.run(  # nosec B603
        [GIT, "tag", "-l", "v0.1.0"],
        cwd=git_repo,
        capture_output=True,
        text=True,
...
def test_dry_run_exits_successfully_without_creating_tags(git_repo):
    ...
    local_tag_check = subprocess.run(  # nosec B603
        [GIT, "tag", "-l", "v0.1.0"],
        cwd=git_repo,
        capture_output=True,
        text=True,
    ...
    remote_tag_check = subprocess.run(  # nosec B603
