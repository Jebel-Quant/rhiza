...
class TestLFSBundleDefinition:
    ...
    @pytest.fixture
    def bundles_data(self, root, lfs_makefile):
        """Load bundles data if available."""
        ...
        try:
            ...
            pytest.skip("PyYAML not installed, skipping bundle tests")

        with open(bundles_file) as f:
            return yaml.safe_load(f)

    def test_lfs_bundle_exists(self, bundles_data):
        """LFS bundle should be defined in template-bundles.yml."""
        assert "bundles" in bundles_data
        assert "lfs" in bundles_data["bundles"]

    def test_lfs_bundle_has_required_fields(self, bundles_data):
        """LFS bundle should have all required fields."""
        lfs_bundle = bundles_data["bundles"]["lfs"]

        assert "description" in lfs_bundle
        assert "files" in lfs_bundle
        assert isinstance(lfs_bundle["files"], list)

    def test_lfs_bundle_includes_makefile(self, bundles_data):
    ...
    def test_lfs_bundle_includes_documentation(self, bundles_data):
    ...
    def test_lfs_bundle_is_standalone(self, bundles_data):
    ...
    def test_lfs_bundle_has_no_requirements(self, bundles_data):
