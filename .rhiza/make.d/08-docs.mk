...
mkdocs-build:: install-uv ## build MkDocs documentation site
	...
	@if [ -f "$(MKDOCS_CONFIG)" ]; then \
	  ...
	  ${UVX_BIN} --with mkdocs-material --with "pymdown-extensions>=10.0" --with mkdocs-minify-plugin mkdocs build \
	    -f "$(MKDOCS_CONFIG)" \
	    -d "$$MKDOCS_OUTPUT_ABS"; \
	else \
...
mkdocs-serve: install-uv ## serve MkDocs site with live reload
	@if [ -f "$(MKDOCS_CONFIG)" ]; then \
	  ${UVX_BIN} --with mkdocs-material --with "pymdown-extensions>=10.0" --with mkdocs-minify-plugin mkdocs serve \
	    -f "$(MKDOCS_CONFIG)"; \
	else \
