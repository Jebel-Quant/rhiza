...
`rhiza_paper.yml` **writes nothing to the repository**, and that is the fix for #1494 rather than an omission. It used to push the compiled PDF to an orphan `paper` branch, which cannot work in a repository that has any `paper/<topic>` branch: git refs are paths, so `refs/heads/paper` and `refs/heads/paper/overview` are mutually exclusive. The push failed outright for exactly the teams most likely to use that prefix — the ones writing a paper — and kept failing after such a branch was merged, until it was also deleted.

Renaming the target (the issue's own first suggestion, and the one it called cheapest) was rejected: it moves the collision instead of removing it, breaks every consumer just as thoroughly as deleting the step, and leaves a stale PDF on the old branch with nothing going red — the silent-staleness shape this repo keeps finding. What made removal the right answer instead is that the branch had stopped being the only durable home: #1598 gave `book` the `paper` prerequisite, `paper_folder` sits inside `docs_dir`, so the PDF ships as a site asset at a stable URL with two guards behind it (`book-nav`, and `test_the_compiled_paper_is_reachable_from_the_book`). The run artifact covers the ephemeral case. The reporter had already done this by hand before filing.

Three consequences worth knowing:

- **The write scope went with it**, in the live workflow *and* in the `github-paper` stub, which carries its own `permissions:` block. Compiling a PDF and uploading an artifact need no write access, so this is a real least-privilege reduction and not bookkeeping. The two halves stay in step because the same release that ships the stub rewrites its `@vX.Y.Z` ref — `[[tool.bumpversion.files]]` globs `bundles/**/.github/workflows/*.yml` — so a consumer never holds a read-only stub pointing at a version that still pushes.
- **A lingering `paper` branch gets a warning, not a deletion.** Removing a consumer's branch from a template workflow is not the template's call, but staying silent is how somebody ends up linking a PDF that quietly stopped changing. The step names the branch and the one command that removes it.
- **The guard reads the shell, not the parsed YAML**, because a `run:` block is an opaque string to a YAML loader. It matches a line that *starts* a `git push` rather than any line mentioning one — this workflow legitimately names `git push origin --delete paper` as advice, and a substring search flags the fix as the bug. That is not hypothetical: it is what the first version of the test did.
