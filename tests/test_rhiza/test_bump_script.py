...
def test_bump_updates_version_no_commit(git_repo, choice, expected_version):
    ...
    result = subprocess.run([shutil.which("sh"), str(script)], cwd=git_repo, input=input_str, capture_output=True, text=True)
...
def test_uncommitted_changes_failure(git_repo):
    ...
    # Input: 1 (patch)
    result = subprocess.run([shutil.which("sh"), str(script)], cwd=git_repo, input="1\n", capture_output=True, text=True)
